...
\begin{theorem} \label{thm:BaseExpansionOfRealNumbers}
    Let $x \in \R$ and $b \in \N$ be given with $b \geq 2$. For any $n \in \N$,
    there exists a unique collection of integers $\set{a_{1}, a_{2}, \dots, a_{n}}$ such that
    \[
    ...
    The sequence $\set{a_{n}}_{n = 1}^{\infty}$ is called the \textbf{base $b$ expansion} of $x$.
...
\begin{proof}
    ...
    where $z_{n} = \floor{b^{n}x}$. Since $0 \in T_{n}$ and $T_{n}$ is bounded above, by
    \Cref{thm:BoundedSubsetOfZHasExtremum}, $T_{n}$ has a greatest element $a_{n} = \max{T_{n - 1}}$ for each $n \in \N$.
    \\[3mm]
    First, we will show the \emph{existence} by mathematical induction on $n$.
    Let $P_{n}:$ The collection of integers $\set{a_{1}, a_{2}, \dots, a_{n}}$ satisfies
    \[
    z_{0} \leq x - \sum_{k = 1}^{n} \frac{a_{k}}{b^{k}} < z_{0} + \frac{1}{b^{n}}.
    \]
    \begin{itemize}
    ...
    $P_{n}$ is true for all $n \in \N$. Thus the \emph{existence} of the collection
    $\set{a_{1}, a_{2}, \dots, a_{n}}$ for each $n \in \N$ is guaranteed.
    \\[6mm]
    ...
    Thus the \emph{uniqueness} of the collection $\set{a_{1}, a_{2}, \dots, a_{n}}$ for each $n \in \N$ is guaranteed.
\end{proof}

\bigskip

\begin{theorem} \label{thm:}
    Let $\set{a_{n}}_{n = 1}^{\infty}$ be the base $b$ expansion of $a \in \R$,
    and $\set{c_{n}}_{n = 1}^{\infty}$ be the base $b$ expansion of $c \in \R$
    for some $b \in \N$ with $b \geq 2$. If $a_{n} = c_{n}$ for all $n \in \N$, then $a = c$.
\end{theorem}

\begin{proof}
    We will prove by contraposition. Suppose that $a \neq c$. Without loss of generality, assume that $a < c$.
    Then we have $c - a > 0$. By \Cref{thm:ArchimedeanProperty}, there exists some $n \in \N$ such that
    \[
    b^{n} > \frac{1}{c - a} \implies \frac{1}{b^{n}} < c - a.
    \]
    Let $z_{0} = \floor{a}$ and $w_{0} = \floor{c}$. Then we have $z_{0} \leq a < z_{0} + 1$ and $w_{0} \leq c < w_{0} + 1$.
    If $z_{0} \neq w_{0}$, then $a_{1} \neq c_{1}$, which contradicts the assumption.
\end{proof}
