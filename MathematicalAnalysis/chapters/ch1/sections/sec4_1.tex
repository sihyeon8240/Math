...
\hyperref[def:LeastUpperBoundProperty]{least upper bound property}. Moreover, $\R$ contains $\Q$ as a subfield, i.e.,
...
\begin{proof}
    Assume that $x, y \in \R$ and $x > 0$. Let $E$ be defined as follows:
    \[
    E \coloneqq \setbuilder*{nx}{nx \leq y, \; n \in \N}.
    \]
    If $E = \emptyset$, which implies that $x > y$, then $n = 1$ satisfies $nx > y$. Otherwise, $E$ is nonempty and bounded
...
\begin{corollary} \label{cor:ArchimedeanPropertyCorollary}
    For any $\veps \in \R^{+}$ and $b \in \R$ with $b > 1$, there exists some $n \in \N$ such that $b^{-n} < \veps$.
\end{corollary}

\begin{proof}
    For any $n \in \N$ and $x \in \R$ with $x \geq -1$, we have
    \[
    (1 + x)^{n} \geq 1 + nx.
    \]
...
\begin{theorem} \label{thm:QIsDenseInR}
    For any $x, y \in \R$ with $x < y$, there exists some $p \in \Q$ such that $x < p < y$.
...
\begin{proof}
    Let $\set*{t_{n}}_{n = 1}^{\infty}$, $\set*{T_{n}}_{n = 1}^{\infty}$
    and $\set*{a_{n}}_{n = 1}^{\infty}$ be defined as follows:
    \begin{itemize}
    ...
    a greatest element by \Cref{thm:BoundedSubsetOfZinZHasExtremum}, which implies that $\set*{a_{n}}_{n = 1}^{\infty}$
