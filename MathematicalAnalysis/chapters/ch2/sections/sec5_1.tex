...
\begin{proof}
    ...
    \begin{itemize}
        \item $d(\mathbf{x}, \mathbf{y}) \geq 0$ is trivial. Also, we have
        \begin{align*}
            d(\mathbf{x}, \mathbf{y}) = 0 &\iff \norm{\mathbf{x} - \mathbf{y}} = 0 \\
            &\iff x_{k} = y_{k} \text{ for all } k = 1, 2, \dots, n \\
            &\iff \mathbf{x} = \mathbf{y}.
        \end{align*}
        \item $d(\mathbf{x}, \mathbf{y}) = d(\mathbf{y}, \mathbf{x})$ is trivial.
...
\begin{definition} \label{def:LimitIsolatedInteriorPoints}
    ...
    For each $r \in \R^{+}$, let $Q_{r}(p)$ be a set defined by
    \[
...
\begin{definition} \label{def:BoundedPerfectDenseSets}
    Let $(X, d)$ be a metric space and $E \subseteq X$.
    \begin{itemize}
        \item $E$ is called \textbf{bounded} if there exists some $p \in X$ and
        $r \in \R^{+}$ such that$E \subseteq N_{r}(p)$.
        \item $E$ is called \textbf{perfect} if any point of $E$ is a limit point of $E$.
        \item $E$ is called \textbf{dense} in $X$ if any point of $X$ is either a limit point of $E$ or a point of $E$.
    \end{itemize}
\end{definition}
...
\begin{theorem} \label{thm:LimitPointNeighborhoodIsInfinite}
    ...
    For any $r \in \R^{+}$, we have $E \cap N_{r}(p)$ is infinite.
\end{theorem}
...
\begin{proof}
    ...
    \[
    N_{\veps}(p) \subset N_{r}(p), \quad E \cap Q_{\veps}(p) = \emptyset.
    \]
...
\begin{proof}
    Assume that $E$ is open. Let $p$ be a limit point of $E^{c}$. Then, for any $r \in \R^{+}$, we have
    \[
    E^{c} \cap Q_{r}(p) \neq \emptyset.
    \]
    ...
    \\[6mm]
    Conversely, assume that $E^{c}$ is closed. Let $p$ be a point of $E$.
