...
\begin{definition} \label{def:NCell}
    Let $(\R^{n}, \norm{\cdot})$ be given for some $n \in \N$. A set $E \subseteq \R^{n}$ defined by
    \[
    E \coloneqq \setbuilder*{\bm{x} = (x_{1}, x_{2}, \dots, x_{n})
    \in \R^{n}}{a_{k} \leq x_{k} \leq b_{k} \text{ for all } k = 1, 2, \dots, n}
    \]
    is called an \textbf{$n$-cell}, where $a_{k}, b_{k} \in \R$ with $a_{k} \leq b_{k}$ for all $k = 1, 2, \dots, n$.
\end{definition}
