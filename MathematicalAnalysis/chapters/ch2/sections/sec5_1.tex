...
\begin{definition} \label{def:KCell}
    ...
    \]
    is called an \textbf{$k$-cell}, where $- \infty < a_{j} \leq b_{j} < + \infty$ for all $j = 1, 2, \dots, k$.
\end{definition}
...
\begin{definition} \label{def:LimitIsolatedInteriorPoints}
    ...
    \begin{itemize}
        ...
        if for any $r \in \R^{+}$, we have $E \cap \widetilde{N}_{r}(p) \neq \emptyset$.
...
\begin{theorem} \label{thm:NeighborhoodsAreOpen}
...
\begin{theorem} \label{thm:LimitPointNeighborhoodContainsInfiniteElements}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item $E \cap \widetilde{N}_{\veps}(p) = \emptyset$.
    \end{itemize}
...
\begin{proof}
    ...
    \[
    E^{c} \cap \widetilde{N}_{\veps}(p) = \emptyset,
    \]
