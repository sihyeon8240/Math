...
\begin{proof}
    ...
    \begin{itemize}
        \item $f(1) = \min X_{1}$, $X_{2} = X_{1} \setminus \set{f(1)}$.
        \item $f(2) = \min X_{2}$, $X_{3} = X_{2} \setminus \set{f(2)}$.
        \item[] $\vdots$
        \item $f(n) = \min X_{n}$, $X_{n + 1} = X_{n} \setminus \set{f(n)}$ for $n \geq 2$.
    \end{itemize}
    By \Cref{thm:WellOrderingProperty}, $X_{n}$ has a least element for all $n \in \N$, so $f$ is well-defined.
