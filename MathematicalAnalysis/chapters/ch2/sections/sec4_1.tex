...
\begin{theorem} \label{thm:InfiniteSetMinusFiniteSetIsInfinite}
    Let $X$ be an infinite set and $Y$ be a finite set. Then $X \setminus Y$ is infinite.
\end{theorem}

\begin{proof}
    Suppose $X \setminus Y$ is finite. Since $Y$ is finite, by \Cref{thm:UnionAndIntersectionOfTwoFiniteSetsAreFinite},
    $(X \setminus Y) \cup Y$ is finite. Then, by \Cref{thm:SubsetOfFiniteSetIsFinite},
    $X \subseteq (X \setminus Y) \cup Y$ is finite, which contradicts the assumption. Thus, $X \setminus Y$ is infinite.
\end{proof}
...
\begin{proof}
    ...
    Then, by \Cref{thm:SubsetOfFiniteSetIsFinite}, $f(J_{n})$ is finite. By
...
\begin{proof}
    ...
    \begin{itemize}
        \item $t_{n} = \min T_{n}, \text{ where } T_{n} \coloneqq \setbuilder*{k \in \N}{n \leq \dfrac{k(k + 1)}{2}}$.
        \item $s_{n} = n - \dfrac{t_{n}(t_{n} - 1)}{2} = n - \dfrac{t_{n}(t_{n} + 1)}{2} + t_{n} \in \N$.
    \end{itemize}
    Since $2n \in T_{n}$, $T_{n} \neq \emptyset$. Then, by \Cref{thm:WellOrderingProperty}, $T_{n}$ has a least element.
    Thus, $t_{n}$ is well-defined. Since $\dfrac{t_{n}(t_{n} - 1)}{2} < n \leq \dfrac{t_{n}(t_{n} + 1)}{2}$, we have
    \[
    t_{n} - s_{n} + 1 = \dfrac{t_{n}(t_{n} + 1)}{2} - n + 1 \geq 1
    \]
    so $t_{n} - s_{n} + 1 \in \N$. Let $g: \N \to \displaystyle \bigcup_{n = 1}^{\infty} X_{n}$ be defined by
    \[
