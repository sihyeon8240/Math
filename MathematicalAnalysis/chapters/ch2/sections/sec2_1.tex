...
Especially, if $X = \vnot$, we define $\abs*{X} = 0$. $X$ is said to be \textbf{infinite} if $X$ is not finite.
...
\begin{proof}
    ...
    \begin{itemize}
        \item \textbf{Base case:} $J_{1} = \set*{1}$ has $\vnot$ and $J_{1}$ as its only subsets. We have
        $J_{1} \hyperref[thm:BijectionEquivalenceRelation]{\sim} J_{1}$ by $f \equiv id_{J_{1}}$, and $\vnot$ is
...
\begin{lemma} \label{lem:SurjectionFromJnImpliesFinite}
...
\begin{theorem} \label{thm:SurjectionFromFiniteSetImpliesFinite}
...
\begin{proof}
    ...
    is surjective. Therefore, by \Cref{lem:SurjectionFromJnImpliesFinite}, $Y$ is finite.
\end{proof}

\bigskip

\begin{theorem} \label{thm:FiniteOrderedSetHasExtremum}
    Let $X$ be a finite nonempty ordered set. Then $X$ has both a least element and a greatest element.
\end{theorem}

\begin{proof}
    Since $X$ is finite and nonempty, there exists a bijection $f: J_{n} \to X$ for some $n \in \N$. The elements
