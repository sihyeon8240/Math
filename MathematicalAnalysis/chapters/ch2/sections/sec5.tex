...
\begin{theorem} \label{thm:StandardMetricOnRn}
    We define a function $d: \R^{n} \times \R^{n} \to \R$ by
    \[
...
\begin{definition} \label{def:LimitIsolatedInteriorPoints}
    Let $(X, d)$ be a metric space and $E \subseteq X$. Let $p \in X$.
    \begin{itemize}
...
\begin{definition} \label{def:OpenClosedSets}
    ...
    \begin{itemize}
        \item $E$ is called \textbf{open} if every point of $E$ is an interior point of $E$.
        \item $E$ is called \textbf{closed} if every limit point of $E$ belongs to $E$.
    \end{itemize}
\end{definition}
...
\begin{proof}
    ...
    We will show that $N_{\veps}(q) \subseteq N_{r}(p)$. Let $s \in N_{\veps}(q)$ be given.
    Then we have $d(q, s) < \veps$. By the \emph{triangle inequality}, we have
    \[
    d(p, s) \leq d(p, q) + d(q, s) < d(p, q) + \veps = r.
    \]
    Therefore, we have $s \in N_{r}(p)$, which implies that $N_{\veps}(q) \subseteq N_{r}(p)$.
    Since $q$ is an arbitrary point of $N_{r}(p)$, every point of $N_{r}(p)$ is an interior point of $N_{r}(p)$.
    Thus, we conclude that $N_{r}(p)$ is open.
\end{proof}
